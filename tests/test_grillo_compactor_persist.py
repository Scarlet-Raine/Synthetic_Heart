...
async def test_persist_compaction_moves_to_archive_and_inserts_memory(monkeypatch):
        ...
        async def fetchall(self):
            ...
            if self.calls == 1:
                return [
                    {
                        "id": 301,
                        "content": "Old food memory A",
                        "tags": json.dumps(["food"]),
                        "timestamp": "2020-01-01",
                    },
                    {
                        "id": 302,
                        "content": "Old food memory B",
                        "tags": json.dumps(["food"]),
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: asyncio.sleep(0, result="zen_chatgpt"),
    )

    # Capture insert_memory calls
    called = {}

    async def fake_insert_memory(
        *,
        content,
        author,
        source,
    ...
    assert any("INSERT INTO ai_diary_archive" in sql for sql, params in queries), (
        f"Archive insert not found in: {queries}"
    )
