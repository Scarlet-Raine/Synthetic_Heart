...
async def send_prompt(engine: str, prompt: str, timeout: int = 300) -> dict:
    ...
    try:
        ...
        response = await asyncio.wait_for(
            asyncio.to_thread(
                lambda: urllib.request.urlopen(req, timeout=timeout).read().decode()
            ),
            timeout=timeout + 30,
        )
...
async def run_test(
    engine: str, prompts: list[str], delays: list[float], label: str
) -> list[dict]:
    ...
    for i, (prompt, delay) in enumerate(zip(prompts, delays)):
        ...
        print(
            f"  [{i + 1}/{len(prompts)}] Sending prompt ({len(prompt)} chars)...",
            end="",
            flush=True,
        )
...
async def main():
    ...
    openrouter_results = await run_test(
        "openrouter", openrouter_prompts, openrouter_delays, "OpenRouter (ChatGPT)"
    )
    ...
    for engine in other_engines:
        ...
        results = await run_test(
            engine, short_prompts, delays, f"{engine} short prompts"
        )
    ...
    for engine, results in all_results.items():
        ...
        print(f"\n--- {engine} ---")
        print(
            f"  Total: {len(results)} | Success: {len(successes)} | Failed: {len(failures)}"
        )
        ...
        if failures:
            ...
            for f in failures:
                print(
                    f"    - Prompt {results.index(f) + 1}: {f['error']} ({f['elapsed']:.1f}s)"
                )

        # Show timing per prompt
        print("  Per-prompt times:")
        for r in results:
    ...
    print(
        f"Total: {total} | Success: {total_success} ({100 * total_success / total:.1f}%) | Failed: {total_fail}"
    )
