...
def test_translate_postgres_sql_preserves_double_precision() -> None:
    """The compat layer must not rewrite an already-Postgres `DOUBLE PRECISION`
    type into `DOUBLE PRECISION PRECISION`, which is a Postgres syntax error
    (``syntax error at or near "PRECISION"``). The runtime Postgres schema
    (scripts/sql/app_main_postgres.sql) is written in Postgres dialect and runs
    through the same translator as MariaDB DDL."""
    sql = """
    ...
    assert translated.count("PRECISION") == 1
    assert "DOUBLE PRECISION" in translated


def test_translate_postgres_sql_still_rewrites_bare_double() -> None:
    """MariaDB-style bare `DOUBLE` columns must still become DOUBLE PRECISION."""
    sql = """
    CREATE TABLE IF NOT EXISTS legacy_metric (
...
def test_translate_postgres_sql_message_map_has_no_reserved_timestamp() -> None:
    """`timestamp` is a PostgreSQL reserved word and cannot be a bare column
    name; the runtime schema must declare the epoch column as `created_at`
    (matching the plugin DDL and the migration normalizer)."""
    sql = """
    ...
    assert "timestamp" not in translated.lower()
    assert "created_at" in translated
    assert translated.count("PRECISION") == 1
...
@pytest.mark.asyncio
async def test_init_db_continues_schema_loop_after_bad_statement(monkeypatch) -> None:
    """A failing PostgreSQL schema statement must be logged and skipped, not
    abort the whole loop. One bad statement (e.g. the old bare `timestamp`
    column) used to stop the schema mid-file, strand every later table, and
    re-fail on every boot (bootloop)."""
    executed: list[str] = []

    class FakeCursor:
        async def execute(self, sql, *args, **kwargs):
            executed.append(sql)
            if "BROKEN_STATEMENT" in sql:
                raise RuntimeError('syntax error at or near "PRECISION"')
    ...
    fake_statements = [
        "CREATE TABLE IF NOT EXISTS fine_one (id BIGSERIAL PRIMARY KEY);",
        "CREATE TABLE IF NOT EXISTS broken_table (id BIGSERIAL PRIMARY KEY, BROKEN_STATEMENT DOUBLE PRECISION);",
        "CREATE TABLE IF NOT EXISTS fine_two (id BIGSERIAL PRIMARY KEY);",
    ]

    monkeypatch.setattr(db_module, "_get_db_type", lambda: "postgres")
    monkeypatch.setattr(db_module, "get_conn_ctx", lambda: FakeConnCtx())
    monkeypatch.setattr(
        db_module, "_load_sql_statements", lambda _path: fake_statements
    )

    await db_module.init_db()

    # All three statements were attempted despite the middle one failing.
    assert len(executed) >= 3
    assert any("fine_one" in sql for sql in executed)
    assert any("broken_table" in sql for sql in executed)
    assert any("fine_two" in sql for sql in executed)
