...
5. **Configure a Cortex (engine)**
   ...
   engine (for example: ``zen_gemini`` or ``gemini_api``).

   -  If using a Zen-driven Cortex (browser adapter), press the
...
- **Cortex Engines**: categories of runtime engines (kinds: ``llm``, ``live``, ``agent``) such as ChatGPT (via Zen), Gemini Live or Agent-based engines. Each engine can provide a short human-readable label at registration time (``ENGINE_LABEL`` or registry ``label``) to explain its intended use. **Note: Only Zen ChatGPT (Legacy) is currently fully functional.**
