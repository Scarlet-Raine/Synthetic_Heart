...
| `SOUL_SPEAKER_IDENTITIES` | Who the people in a chat are, stated outright and followed exactly, e.g. `Scar - he/him, the human; 2B - she/her, the persona, me`. It reaches both extractors AND the profile compiler, because the compiler is the stage that decides which name and gender the standing `[About the person you're talking to]` profile carries. The persona's own lines are labelled `<name> (the persona)` in the transcript the extractors read (the interfaces cache them under the bare `self` label), so a line can never be read as the human's by mistake. Leave empty to keep the previous behaviour. |
