...
| Live deployment | The running instance **hot-picked the plugin up without a restart** (its plugin scan registered `home_assistant` with all 14 actions at 13:27). It is inert so far: the injection returns logged at 14:57-15:14 carry no `home` key, because a config value written straight to the `config` table is not in the live process's registry. A config page load or a restart activates it. |
